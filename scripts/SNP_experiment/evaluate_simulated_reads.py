...
def get_best_match(corrected_reads, reference_transcripts, outfolder, params):
    ...
    out_file.write("{0},{1},{2},{3},{4},{5},{6}\n".format("q_acc", "ref_acc", "total_errors", "error_rate", "abundance", "mutation_present", "minor"))
    ...
    for q_acc in errors_container:
        ...
        minor = 1 if q_acc_mod == "sim|sim|2" else 0   # true_abundance <= max(list(original_abundances.values())) else 0
        mut_present = 1 if q_acc_mod ==  errors_container[q_acc][1] else 0
        if mut_present == 1 and minor == 1:
            # print("here",mut_present,minor )
            minor_mut_retained = 1
        out_file.write("{0},{1},{2},{3},{4},{5},{6}\n".format(q_acc, best_ref_acc, best_ref_ed, round(100*best_ref_ed/float(ref_len),4), true_abundance, mut_present, minor )) 
