...
#     do
#         python $experiment_dir/generate_ont_reads.py $outbase/$id/biological_material_abundance.fa $outbase/$depth/$id/reads.fq $depth > /dev/null

#         python $inbase/isONcorrect3 --fastq $outbase/$depth/$id/reads.fq   --outfolder $outbase/$depth/$id/isoncorrect/ --k 7 --w 10 --xmax 80  > /dev/null            
#         python $eval_dir/evaluate_simulated_reads.py  $outbase/$depth/$id/isoncorrect/corrected_reads.fastq  $outbase/$id/biological_material.fa $outbase/$depth/$id/isoncorrect/evaluation > /dev/null
#         echo -n  $id,approx,$depth,$mut_rate,&& head -n 1 $outbase/$depth/$id/isoncorrect/evaluation/summary.csv 
#         echo -n  $id,approx,$depth,$mut_rate, >> $summary_file && head -n 1 $outbase/$depth/$id/isoncorrect/evaluation/summary.csv >> $summary_file
#         awk -F "," -v awk_id=$id -v awk_depth=$depth -v awk_p=$mut_rate  '{if (NR!=1) {print awk_id",approx,"awk_depth","awk_p","$0}}'  $outbase/$depth/$id/isoncorrect/evaluation/results.csv >> $results_file

        
#         # for line in $(cat $outbase/$depth/$id/isoncorrect/evaluation/results2.csv ); do
#         #         # echo "tester: $line"
#         #         echo -n  $id$'\t'$depth$'\t'$mut_rate$'\t' >> $results_file && echo $line >> $results_file
#         # done
#         # # cat $outbase/$depth/$id/isoncorrect/evaluation/results2.csv >> $results_file


#         fastq2fasta $outbase/$depth/$id/reads.fq $outbase/$depth/$id/reads.fa
#         python $eval_dir/evaluate_simulated_reads.py   $outbase/$depth/$id/reads.fa $outbase/$id/biological_material.fa $outbase/$depth/$id/evaluation_reads > /dev/null
#         echo -n  $id,original,$depth,$mut_rate,&& head -n 1 $outbase/$depth/$id/evaluation_reads/summary.csv 
#         echo -n  $id,original,$depth,$mut_rate, >> $summary_file && head -n 1 $outbase/$depth/$id/evaluation_reads/summary.csv  >> $summary_file
#         awk -F "," -v awk_id=$id -v awk_depth=$depth -v awk_p=$mut_rate '{if (NR!=1) {print awk_id",original,"awk_depth","awk_p","$0}}'  $outbase/$depth/$id/evaluation_reads/results.csv >> $results_file
...
#             continue
#         else
#             python $inbase/isONcorrect3 --fastq $outbase/$depth/$id/reads.fq   --outfolder $outbase/$depth/$id/isoncorrect_exact/ --k 7 --w 10 --xmax 80 --exact   > /dev/null            
#             python $eval_dir/evaluate_simulated_reads.py  $outbase/$depth/$id/isoncorrect_exact/corrected_reads.fastq  $outbase/$id/biological_material.fa $outbase/$depth/$id/isoncorrect_exact/evaluation > /dev/null
#             echo -n  $id,exact,$depth,$mut_rate,&& head -n 1 $outbase/$depth/$id/isoncorrect_exact/evaluation/summary.csv 
#             echo -n  $id,exact,$depth,$mut_rate, >> $summary_file && head -n 1 $outbase/$depth/$id/isoncorrect_exact/evaluation/summary.csv >> $summary_file
#             awk -F "," -v awk_id=$id -v awk_depth=$depth -v awk_p=$mut_rate '{if (NR!=1) {print awk_id",exact,"awk_depth","awk_p","$0}}'  $outbase/$depth/$id/isoncorrect_exact/evaluation/results.csv >> $results_file
...
#         # corrected_reads_fastq=$outbase/$depth/$id/"isoncorrect"/"corrected_reads.fastq"
#         # database=$outbase/$id/"biological_material.fa"
#         # error_plot=$outbase/$depth/$id/"error_rates_"$depth".pdf" 
#         # python $tmp_experiment_dir/get_error_rates.py  $database $outbase/$depth/$id/reads.fq  $corrected_reads_fastq  > $error_rates_file #$outbase/$depth/$id/isoncorrect/evaluation #&> /dev/null
...
#         # minimap2 -ax splice --eqx $database  -uf -k14 $outbase/$depth/$id/reads.fq > $original_reads_mappings
#         # # ###################

#         # # # Evaluate chnage in expression levels compared to true expression
#         # abundance_file=$outbase/$depth/$id/"abundance.csv"
#         # > $abundance_file
...
#         # abundance_plot=$outbase/$depth/$id/"abundance.pdf" 
