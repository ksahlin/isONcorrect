...
# Simulate reads
echo  python $experiment_dir/simulate_reads.py $database_filtered $outbase/$id/$depth/reads.fq $depth
python $experiment_dir/simulate_reads.py $database_filtered $outbase/$id/$depth/reads.fq $depth #&> /dev/null
###############
...
echo python $isonclust_dir/isONclust --fastq $outbase/$id/$depth/reads.fq --outfolder $outbase/$id/$depth/isonclust/ --k 13 --w 25 --q 6.0 --t $cores
...
python $isoncorrect_dir/run_isoncorrect --keep_old --t $cores --fastq_folder $outbase/$id/$depth/isonclust/fastq  --outfolder $outbase/$id/$depth/isoncorrect/ --k 7 --w 10 --xmax 80 --exact_instance_limit 100  # &> /dev/null            
###############################


# Evaluate individually corrected clusters    
> $corrected_reads_fastq
# FILES="$outbase/$id/$depth/isoncorrect"/*/corrected_reads.fastq
# find $outbase/1/$depth/isoncorrect/*
for folder in $(find $outbase/1/$depth/isoncorrect/*  -type d); #c_id in $(seq 0 1 1000) $"${FILES[@]}"
    do 
        # echo $folder
        ...
        if [ -f $folder/corrected_reads.fastq ]; then
            ...
            python $eval_dir/evaluate_simulated_reads.py --no_ref_sim $outbase/1/$depth/isonclust/fastq/$cl_id.fastq $database_filtered $folder/evaluation_reads  #> /dev/null
