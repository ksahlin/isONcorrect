...
def main(args):
    ...
    gff_transcript_accesions = read_file(args.gff_file)
    fasta = { acc : seq for acc, (seq,qual) in readfq(open(args.fasta, 'r'))}
    fasta_tr_ids = { acc.split("|")[2] : acc for acc, seq in fasta.items()}

    output_seqs = {}

    for tr_id in gff_transcript_accesions:
        if tr_id in fasta_tr_ids:
            full_acc = fasta_tr_ids[tr_id]
            seq = fasta[full_acc]
            output_seqs[full_acc] = seq

    outfile = open(args.outfile,"w")
    for acc, seq in output_seqs.items():
        outfile.write(">{0}\n{1}\n".format(acc, seq))
...
if __name__ == '__main__':
    ...
    parser.add_argument('gff_file', type=str, help='file. ')
