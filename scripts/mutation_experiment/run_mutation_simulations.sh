...
do 
    ...
    do
        ...
        do
            ...
            python /users/kxs624/Documents/workspace/isONcorrect/isONcorrect3 --fastq $outbase/$depth/$id/$p/reads.fq   --outfolder $outbase/$depth/$id/$p/isoncorrect/ --k 7 --w 10 --xmax 80 --exact   > /dev/null            
            python $eval_dir/evaluate_simulated_reads.py  $outbase/$depth/$id/$p/isoncorrect/corrected_reads.fastq  $outbase/$depth/$id/isoforms.fa  $outbase/$depth/$id/$p/isoncorrect_exact/evaluation --deal_with_ties > /dev/null
