...
def simulate_reads( args, isoforms ):
    ...
    with_exon = [(isoforms[0][0] + "_{0}".format(i), isoforms[0][1])  for i in range(int(round(args.nr_reads*(1-args.probs))))]
    without_exon = [(isoforms[1][0] + "_{0}".format(i), isoforms[1][1]) for i in range(int(round(args.nr_reads*args.probs)))]
    isoforms_generated =  with_exon + without_exon
...
def generate_transcripts(transcript, ref_path, exon_length):
    transcripts_out = open(os.path.join(args.outfolder,"isoforms.fa"), "w")
    # only two
    transcripts_out.write(">sim|sim|{0}\n{1}\n".format("1", transcript))
    
    exon_start_position = random.randrange(75,125)
    mutated_transcript = transcript[ : exon_start_position] + transcript[ exon_start_position + exon_length :]
...
def main(args):
    ...
    if args.sim_exon_removal:
        generate_transcripts(args.ref, args.outfolder,args.exon_length)
        sys.exit()
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sim_exon_removal',  action="store_true", help='Simulate mutation')
    parser.add_argument('--exon_length', type=int, default = 10, help='Outfolder path')
